<script lang="ts">
    ...
    async function cargar() {
        cargando = true;
        const sueldos = (await query("SELECT fecha, periodo FROM ingreso WHERE perfil_id=1 AND categoria='Salario' AND tipo='Sueldo' AND periodo IS NOT NULL ORDER BY fecha")) as any[];
        ...
        const gastos = (await query(`SELECT g.fecha, g.monto, g.categoria_id, COALESCE(g.subcategoria_id, m.subcategoria_id) AS scid FROM gasto g LEFT JOIN mapeo_detalle m ON m.perfil_id = g.perfil_id AND m.detalle = g.detalle WHERE g.perfil_id = 1`)) as any[];
        const hab = (await query(`SELECT scid, categoria_id, COUNT(*) AS c FROM (SELECT COALESCE(g.subcategoria_id, m.subcategoria_id) AS scid, g.categoria_id FROM gasto g LEFT JOIN mapeo_detalle m ON m.perfil_id=g.perfil_id AND m.detalle=g.detalle WHERE g.perfil_id=1) GROUP BY scid, categoria_id`)) as any[];
...
{:else}
    ...
    <div class="table-container">
        <table>
            <thead><tr><th>Categoría</th><th>n-2</th><th>n-1</th><th>Presup</th><th>Real</th><th>Desvío</th></tr></thead>
            <tbody>
                {#each consolidado as c (c.cat)}
                    <tr><td><strong>{c.cat}</strong></td><td class="num">{peso(c.n2)}</td><td class="num">{peso(c.n1)}</td><td class="num">{peso(c.presup)}</td><td class="num">{peso(c.real)}</td><td class={claseEstado(c.estado)}>{c.estado}</td></tr>
                {/each}
            </tbody>
            <tfoot><tr><td><strong>Total</strong></td><td class="num">{peso(totales.n2)}</td><td class="num">{peso(totales.n1)}</td><td class="num">{peso(totales.presup)}</td><td class="num">{peso(totales.real)}</td><td></td></tr></tfoot>
        </table>
    </div>
    ...
    <div class="table-container">
        <table>
            <thead><tr><th>Subcategoría</th><th>n-2</th><th>n-1</th><th>Presup</th><th>Real</th><th>Desvío</th></tr></thead>
            <tbody>
                {#each grupos as g (g.cat)}
                    ...
                    {#each g.rows as f (f.scid ?? 'null')}
                        <tr>
                            ...
                            <td class="num">{peso(f.n2)}</td><td class="num">{peso(f.n1)}</td>
                            <td class="num">{#if f.scid != null}<input class="presup" type="number" value={f.presup || ''} onchange={(e) => guardarPresup(f.scid, e.currentTarget.value)} />{:else}—{/if}</td>
                            <td class="num">{peso(f.real)}</td><td class={claseEstado(f.estado)}>{f.estado}</td>
                        </tr>
                    {/each}
                {/each}
            </tbody>
...
<style>
    :global(body) { max-width: 1000px; margin: 0 auto; padding: 16px; }
    ...
    th, td { padding: 8px; text-align: left; overflow: hidden; text-overflow: ellipsis; }
    
    th:first-child, td:first-child { width: 25%; }
    th:not(:first-child), td:not(:first-child) { width: 15%; text-align: right; }
    
    .ind { padding-left: 20px; }
    tr.cat td { background: var(--surface-2); font-weight: 700; }
    input.presup { width: 100%; max-width: 80px; text-align: right; }
