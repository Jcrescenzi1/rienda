<script lang="ts">
	...
	let buckets = $state<{ renta: string; v: number; pct: number }[]>([]);
	// % Invertido = 100% - % Líquido. El bucket 'Liquido' de calcularTenencia() ya
	// junta Caja (ARS/USD) + activos con renta='Liquido' (p.ej. FCI money market):
	// eso es lo que NO cuenta como "invertido". No se deriva de invertidoUSD/totalUSD
	// (t.invertidoUSD = hold.reduce mercadoUSD) porque ese total sí incluye esos
	// activos líquidos como si fueran inversión.
	const pctInvertido = $derived(1 - (buckets.find((b) => b.renta === 'Liquido')?.pct ?? 0));
...
{:else}
	...
	<div class="resumen">
		...
		<div class="card">
			<span>% Invertido</span>
			<strong><CountUp value={pctInvertido * 100} format={(n) => n.toFixed(1) + '%'} /></strong>
		</div>
