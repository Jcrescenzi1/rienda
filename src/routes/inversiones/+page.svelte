<script lang="ts">
	...
	let fCuenta = $state<string>(''); // id o 'nueva'
	let fCuentaNueva = $state('');
	let fActivo = $state<string>(''); // id o 'nuevo'
	...
	let naTicker = $state('');
	let naNombre = $state('');
	let naTipo = $state('Accion');
	let naRenta = $state('Variable');
	let naMoneda = $state('USD');
	let fMsg = $state('');

	let editId = $state<number | null>(null);
	let editPrecio = $state<number | null>(null);

	function abrirEdit(h: any) {
		editId = h.id;
		editPrecio = h.precioActual;
	}
	...
	async function guardarOperacion() {
		...
		if (fActivo === 'nuevo' && (!naTicker.trim() || !naNombre.trim())) return (fMsg = 'Completá ticker y nombre del activo nuevo');
		...
		try {
			...
			if (fActivo === 'nuevo') {
				const r = (await query(
					'INSERT INTO activo (perfil_id,ticker,nombre,tipo,renta,moneda) VALUES (1,?,?,?,?,?) RETURNING id',
					[naTicker.trim(), naNombre.trim(), naTipo, naRenta, naMoneda]
				)) as any[];
				activoId = r[0].id;
				monedaA = naMoneda;
			} else {
				activoId = Number(fActivo);
				monedaA = activosList.find((x) => String(x.id) === fActivo).moneda;
			}

			// Venta: validar tenencia
			if (fOp === 'Venta') {
				...
				if (neto[0].n + 1e-9 < u) return (fMsg = `No podés vender ${u}; tenés ${neto[0].n.toFixed(2)} unidades`);
			}

			const precio = m / u;
			await query(
				'INSERT INTO transaccion (perfil_id,activo_id,cuenta_inversion_id,fecha,operacion,unidades,precio,valor_dolar) VALUES (1,?,?,?,?,?,?,?)',
				[activoId, cuentaId, fFecha, fOp, u, precio, fValorDolar]
			);
			// Actualiza precio de mercado del activo con el último operado
			await query('UPDATE activo SET precio_actual=?, precio_actualizado_en=? WHERE id=?', [precio, fFecha, activoId]);

			fMsg = `${fOp} de ${naNombre || activosList.find((x) => String(x.id) === fActivo)?.nombre || 'activo'} guardada ✅`;
			fUnidades = null;
			fMonto = null;
			fActivo = '';
			fCuentaNueva = '';
	...
	async function borrarTx(id: number, activoId: number) {
...
{:else}
	...
	{#if showForm}
		<div class="form">
			...
			{#if fCuenta === 'nueva'}
				<label>Nombre cuenta<input bind:value={fCuentaNueva} placeholder="Ej: Balanz" /></label>
			{/if}
			...
			{#if fActivo === 'nuevo'}
				<div class="nuevo">
					<label>Ticker<input bind:value={naTicker} placeholder="Ej: AAPL" /></label>
					<label>Nombre<input bind:value={naNombre} placeholder="Ej: Apple" /></label>
					<label>Tipo
						<select bind:value={naTipo}><option>Accion</option><option>CEDEAR</option><option>Bono</option><option>ON</option><option>FCI</option><option>Indice</option></select>
					</label>
					<label>Renta
						<select bind:value={naRenta}><option>Variable</option><option>Fija</option><option>Mixta</option><option>Liquido</option></select>
					</label>
			...
			<label>Unidades<input type="number" step="any" min="0" bind:value={fUnidades} /></label>
			<label>Monto ({monedaOp})<input type="number" step="any" min="0" bind:value={fMonto} placeholder="lo que entró/salió de tu cuenta" /></label>
			<label>Valor dólar (para conversión a USD)<input type="number" step="any" min="0" bind:value={fValorDolar} /></label>

			{#if precioCalc > 0}<p class="hint">Precio efectivo: {money(precioCalc, monedaOp, 4)} por unidad</p>{/if}
			...
			{#each cartera as h (h.nombre + h.tipo + h.moneda)}
				<tr>
					...
					<td class="num precioedit">
						{#if editId === h.id}
							<input type="number" step="any" bind:value={editPrecio}
	...
	<p class="nota">≈USD al dólar más reciente (${nf(dolar)}). Realizado atribuido al año de cierre, convertido a USD al dólar de cada operación.</p>
{/if}
