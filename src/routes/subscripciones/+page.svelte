<script lang="ts">
	...
	async function cargar() {
		...
		subs = await query(`
			SELECT s.id, s.nombre, s.monto, s.moneda, s.activa,
			       s.categoria_id, c.nombre AS categoria, t.nombre AS tarjeta
			FROM suscripcion s
		...
		registradas = r;
		const p = (await query(
			"SELECT COALESCE(SUM(monto),0) AS t FROM suscripcion WHERE perfil_id=1 AND activa=1 AND moneda='ARS'"
		)) as any[];
		presupApps = p[0].t;
	}
	...
	async function agregar() {
		...
		await query(
			'INSERT INTO suscripcion (perfil_id,nombre,monto,moneda,categoria_id,tarjeta_id) VALUES (1,?,?,?,?,?)',
	...
	async function toggleActiva(s: any) {
		await query('UPDATE suscripcion SET activa=? WHERE id=?', [s.activa ? 0 : 1, s.id]);
		await cargar();
	}
	...
	async function confirmarDisparo(s: any) {
		...
		try {
			const g = (await query(
				"INSERT INTO gasto (perfil_id,fecha,monto,moneda,categoria_id,detalle,medio,cuotas) VALUES (1,?,?,?,?,?,'debito',1) RETURNING id",
				[dFecha, m, s.moneda, s.categoria_id, s.nombre]
			)) as any[];
			await query('INSERT INTO suscripcion_registro (suscripcion_id,gasto_id,periodo) VALUES (?,?,?)', [
				s.id, g[0].id, periodo
			]);
			disparando = null;
			mensaje = `"${s.nombre}" registrada en ${periodo} ✅`;
			await cargar();
		} catch (e: any) {
...
<table>
	<thead>
		<tr><th>Nombre</th><th>Monto</th><th>Tarjeta</th><th>Estado del mes</th><th>Baja</th></tr>
	</thead>
	<tbody>
		{#each subs as s (s.id)}
			<tr class:inactiva={!s.activa}>
				<td>{s.nombre}</td>
				...
				<td><button class="sec" onclick={() => toggleActiva(s)}>{s.activa ? 'Desactivar' : 'Activar'}</button></td>
			</tr>
		{/each}
		{#if subs.length === 0}
			<tr><td colspan="5" class="vacio">No hay suscripciones. Agregá una abajo.</td></tr>
		{/if}
	</tbody>
</table>
...
<div class="form">
	...
	<label>Moneda
		<select bind:value={nMoneda}><option>ARS</option><option>USD</option></select>
	</label>
	<label>Categoría
		<select bind:value={nCatId}>
			{#each categorias as c (c.id)}<option value={c.id}>{c.nombre}</option>{/each}
		</select>
	</label>
	<label>Tarjeta (opcional, solo referencia)
		<select bind:value={nTarjetaId}>
			<option value={null}>— ninguna —</option>
			{#each tarjetas as t (t.id)}<option value={t.id}>{t.nombre}</option>{/each}
		</select>
	</label>
