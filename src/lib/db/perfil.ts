...
// Crea el perfil id=1 con el nombre dado, carga las categorías genéricas y
// los datos macro de arranque (dólar/inflación de fallback).
...
export async function crearPerfil(nombre: string): Promise<void> {
	const limpio = nombre.trim();
	if (!limpio) throw new Error('El nombre no puede estar vacío.');

	await query('INSERT INTO perfil (id, nombre) VALUES (1, ?)', [limpio]);
